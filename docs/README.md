...
├── docs/
│   └── README.md                     # Source documentation (this file)
...
├── vp-out/                           # Generated Validated Pattern (this directory)
...
│   ├── values-secret.yaml.template   # Secrets template — fill in and load before deploying
│   ├── Makefile / Makefile-common    # VP make targets
│   ├── pattern.sh                    # VP utility container wrapper
│   ├── pattern-metadata.yaml         # Generation provenance
│   └── charts/                       # Charts wired as ArgoCD applications
...
│   ├── openshift-registry/           # Integrated registry config (for BuildConfig push)
│   └── pattern-secrets/              # ExternalSecrets for provider API keys + SSH
├── image-builder-charts/             # Build-time charts (applied manually)
│   └── helm/
│       ├── nemoclaw-imagestream/     # NemoClaw sandbox image BuildConfig
│       ├── nemoclaw-cli-imagestream/ # NemoClaw CLI image BuildConfig
│       └── openshell-gateway-image/  # Bootc gateway VM image + CDI DataSource
├── overrides/
│   └── openshell-saw.yaml            # Default sandbox values (provider, model, namespace_mode)
├── qs-out/                           # Generated Quickstart Helm chart (this directory)
│   ├── chart/                        # Self-contained Helm chart for direct install
│   │   ├── Chart.yaml
│   │   ├── values.yaml               # Config defaults + secrets placeholders
│   │   └── templates/                # All component templates in one chart
│   ├── scripts/
│   │   └── create-secrets.sh         # Interactive secret creation helper
│   └── README.md                     # Quickstart install instructions
└── scripts/                          # Runtime utilities
    ├── generate-keys.sh              # SSH keypair generation
    ├── openshell-saw-create.sh       # Sandbox provisioning logic
    ├── openshell-saw-gui.sh          # Web UI port-forward
...
<!-- vp-only -->
- **Validated Pattern flow:** Computed from `global.clusterDomain` injected by ArgoCD at deploy time. No manual configuration required.
<!-- end -->
